...
describe('CrearJugador', () => { // Test Suites
  it('debe manejar la creación de jugador con éxito', async () => {
    ...
    const { getByText, getByTestId } = render(<CrearJugador />);
    ...
    await waitFor(() => {
      const mensajeRespuesta = getByText('Jugador creado exitosamente.');
...
describe('CrearJugador', () => {
  it('debe manejar errores al crear el jugador', async () => {
    ...
    // Simula el envío del formulario
    const crearJugador = getByText('Crear Jugador',{ selector: 'button.btn.btn-primary' });
    fireEvent.click(crearJugador);
